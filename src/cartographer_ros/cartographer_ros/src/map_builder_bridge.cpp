...
return 是否有匹配结果；
*/
bool MapBuilderBridge::Relocate( float cutoff, const cartographer::sensor::PointCloud& point_cloud,
        cartographer::transform::Rigid2d* best_pose_estimate, float* best_score)
{
  LOG(INFO) << "Relocating...";
    return map_builder_->pose_graph()->PerformGlobalLocalization(cutoff, point_cloud, best_pose_estimate, best_score);
}
