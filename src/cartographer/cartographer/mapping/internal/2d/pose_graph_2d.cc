...
LOG(INFO)<< "Start creating matchers11";
auto pool = std::make_unique<common::ThreadPool>(
    std::thread::hardware_concurrency());
assert(pool != nullptr); 
LOG(INFO)<< "Start creating matchers22";
const sensor::PointCloud filtered_point_cloud = cartographer::sensor::VoxelFilter(point_cloud,0.05); 
//为每一幅 Submap 创建匹配器，这一步需要在线程池里加速执行。
...
LOG(INFO)<< "Start creating matchers size : "<<submap_size;
size_t index = 0;
for (const auto& submap_id_data : data_.submap_data) {
    // if (submap_id_data.id.trajectory_id != 0) {
    //   LOG(INFO)<< "continue : "<<submap_id_data.id.trajectory_id;
    //     continue;
    // }
